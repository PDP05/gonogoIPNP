        ...
        while toc(start_nogoTrial) < FRW_duration
            ...
            switch true
                case max(nogoLickCount) > nogo_threshold
                    ...
                    switch puff
                        case true
                            deliverPuff(); % adapt if the valves are connected to the NI board, here it sends a HIGH signal though a port set in deliverPuff, works if the valves are connected to the Arduino.
    ...
    case trial_type <= go_probability || guarantee_go
        ...
        while toc(start_goTrial) < FRW_duration
            ...
            if  max(goLickCount) < lick_threshold
                ...
                deliverWater(); % adapt if the valves are connected to the NI board, here it sends a HIGH signal though a port set in deliverWater, works if the valves are connected to the Arduino.
            end
